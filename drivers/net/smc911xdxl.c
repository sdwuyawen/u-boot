...
#ifdef CONFIG_NET_MULTI		/* 定义了多网卡支持 */
...
static int smc911x_eth_init(struct eth_device* dev, bd_t* bis)
{
	...
	*/
	...
	/* 把MAC地址从环境变量复制到dev->enetaddr，在eth.c中的eth_initialize()调用eth_write_hwaddr()已经做了 */
//	/* 从bd获取MAC地址 */
//	memcpy(dev->enetaddr, bis->bi_enetaddr, 6);

	val = reg_read(BYTE_TEST);	/* 读取BYTE_TEST，应该是0x87654321,见LAN9220数据手册 */
	if (val != 0x87654321) {
	...
	{
		printf(DRIVERNAME ": Matched chip ID %04lx\n", val);
	}
	...
	if (smx911x_handle_mac_address(bis))
		goto err_out;
...
static int smc911x_eth_write_hwaddr (struct eth_device *dev)
{
	
	unsigned long addrh, addrl;
	unsigned char *m = dev->enetaddr;

	/* 把dev->enetaddr[0-5]中的mac信息写入LAN9220 */
	if ((m[0] | m[1] | m[2] | m[3] | m[4] | m[5])) 
	{
		addrl = m[0] | m[1] << 8 | m[2] << 16 | m[3] << 24;
		addrh = m[4] | m[5] << 8;
	...
	printf("smc911x_eth_write_hwaddr " DRIVERNAME ": MAC %02x:%02x:%02x:%02x:%02x:%02x\n",
		m[0], m[1], m[2], m[3], m[4], m[5]);
...
int smc911x_initialize(u8 dev_num, int base_addr)
{
	...
	/* 申请内存后，dev结构中的数据是不确定的，最好全清0 */
	memset(dev, 0x00, sizeof(struct eth_device));		/* 把dev结构全清0 */
	dev->enetaddr[0]=0x01;
	dev->enetaddr[1]=0x02;
	dev->enetaddr[2]=0x03;
	dev->enetaddr[3]=0x04;
...
//	memcpy(dev->enetaddr, bis->bi_enetaddr, 6);	/* 改到smc911x_eth_init中从bd获取MAC地址 */
	...
	dev->write_hwaddr = smc911x_eth_write_hwaddr;		/* 新增加smc911x_eth_write_hwaddr，在eth.c中eth_write_hwaddr()被调用 */
...
#else	/* 单网卡时，可以把eth.c中的eth_init,eth_send,eth_halt,eth_rx直接定义在smc911xdxl.c这种网卡驱动中，但是不规范 */
