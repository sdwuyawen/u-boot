		   ...
		   int eth_number)
{
	...
	if (memcmp(env_enetaddr, "\0\0\0\0\0\0", 6)) {	/* 从环境变量获取的MAC不是全0 */
		if (memcmp(dev->enetaddr, "\0\0\0\0\0\0", 6) &&	/* 该dev中的enetaddr已经不全是0 */
			memcmp(dev->enetaddr, env_enetaddr, 6)) {	/* 环境变量和dev中的enetaddr不匹配 */
		...
		memcpy(dev->enetaddr, env_enetaddr, 6);	/* 把从环境变量获取的env_enetaddr复制到dev的结构中 */
	}

	if (dev->write_hwaddr &&
		!eth_mac_skip(eth_number) &&
		is_valid_ether_addr(dev->enetaddr)) {
		debug("dev->write_hwaddr = %08x\r\n", dev->write_hwaddr);
...
int eth_initialize(bd_t *bis)
{
	...
	if (!eth_devices) {	/* 没有找到eth_device */
		...
		debug("List all eth_devices\r\n");	/* by wu */
		bootstage_mark(BOOTSTAGE_ID_NET_ETH_INIT);
		do {				/* 列出所有eth_device设备 */
			if (dev->index)
				puts (", ");

			printf("%s", dev->name);	/* 打印设备名称 */
